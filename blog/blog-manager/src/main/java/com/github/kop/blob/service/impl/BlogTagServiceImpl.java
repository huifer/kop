...
public class BlogTagServiceImpl extends ServiceImpl<BlogTagMapper, BlogTag>
  ...
  public int add(BlogTagAddReq req) {
    ...
    return baseMapper.insert(BlogTag.builder().tagName(req.getTagName()).build());
  }
  ...
  private TagListRes conv(BlogTag blogTag) {
    return TagListRes.builder().tagId(blogTag.getTagId()).tagName(blogTag.getTagName()).build();
  }

  protected static final class TagCreateAndUpdateValidate implements CreateValidate<BlogTagAddReq> {
    @Override

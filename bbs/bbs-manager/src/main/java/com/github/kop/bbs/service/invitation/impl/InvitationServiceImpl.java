...
public class InvitationServiceImpl implements InvitationService {
  ...
  public IPage<InvitationQueryResp> page(Long categoryId, Long page, Long size) {
    IPage<Invitation> invitationIPage =
        this.invitationRepository.page(categoryId, null, page, size);
    ...
    if (ObjectUtils.isNotEmpty(invitationIPage.getRecords())) {
      List<Long> userIds =
          invitationIPage
              .getRecords()
              .stream()
              .map(Invitation::getCreateUserId)
    ...
    return invitationIPage.convert(
        invitation ->
            InvitationQueryResp.builder()
                ...
                .tags(
                    ObjectUtils.isEmpty(invitation.getTagStr())
                        ? Collections.emptyList()
                        : Arrays.asList(invitation.getTagStr().split(",")))
  ...
  public IPage<InvitationAuditRes> auditList(Long categoryId, Long page, Long pageSize) {
    IPage<Invitation> invitationIPage =
        this.invitationRepository.page(categoryId, null, page, pageSize);
    Map<Long, String> userNameMap = Collections.emptyMap();
  ...
  public boolean audit(Long userId, InvitationAuditReq req) {
    ...
    return auditServiceFactory
        .factory(AuditTypeEnum.INVITATION)

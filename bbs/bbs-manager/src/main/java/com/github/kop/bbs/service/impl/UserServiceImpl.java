...
public class UserServiceImpl  implements UserService {
    ...
    public Boolean updateUser(UpdateUserReq req) {
        ...
        if(ObjectUtils.isNotEmpty(req.getPassword())){
            String salt = RandomStringUtils.randomAlphanumeric(10);
            user.setSalt(salt);
            user.setPassword(DigestUtils.md5DigestAsHex((req.getPassword() + salt).getBytes(StandardCharsets.UTF_8)));
        }
    ...
    public UserLoginRes login(LoginUserReq req) {
        ...
        if (!user.getPassword().equals(DigestUtils.md5DigestAsHex((req.getPassword() + user.getSalt()).getBytes(StandardCharsets.UTF_8)))) {

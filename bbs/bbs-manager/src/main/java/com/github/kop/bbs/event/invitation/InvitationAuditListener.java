...
public class InvitationAuditListener implements ApplicationListener<InvitationAudit> {
  ...
  @Resource private BbsConfiguration bbsConfiguration;

  @Resource private InvitationService invitationService;

  @Resource private MidUserRoleService midUserRoleService;

  @Resource private InvitationAuditLogService invitationAuditLogService;
  ...
  public void onApplicationEvent(InvitationAudit event) {
    ...
    // 更新帖子审核状态
    if (AuditStatusEnum.PASS
        .getCode()
        .equals(invitationAuditEventBo.getAuditStatusEnum().getCode())) {
      // 根据状态和id获取帖子,如果帖子已经审核通过了,就不需要执行了
      Invitation invitation =
          invitationService.findByIdAndAuditStatus(
              invitationAuditEventBo.getSid(), AuditStatusEnum.PASS.getCode());
      ...
      // 当前可以审核的人员数量
      int auditUserCount =
          auditUserList.stream().map(MidUserRole::getUserId).collect(Collectors.toSet()).size();
      invitationService.updateAuditStatus(
          auditUserCount, bbsConfiguration.getAuditThreshold(), invitationAuditEventBo.getSid());
    }
    invitationAuditLogService.insert(
        InvitationAuditLog.builder()

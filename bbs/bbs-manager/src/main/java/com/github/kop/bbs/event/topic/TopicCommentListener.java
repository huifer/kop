...
public class TopicCommentListener implements ApplicationListener<TopicCommentEvent> {
  ...
  @Resource private TopicCommentCountService topicCommentCountService;
  ...
  public void onApplicationEvent(@NotNull TopicCommentEvent event) {
    ...
    // 到达阈值执行更新操作
    if (sumCount > bbsConfiguration.getTopicCommentSumThreshold()
        || duration.toMinutes() >= bbsConfiguration.getTopicCommentCheckTime()) {
      ...
      commentCountMap.forEach(
          (topicId, count) -> {
            updateBoArrayList.add(
                TopicCommentUpdateBo.builder()

...
CREATE TABLE `bbs_category` (
  ...
  `parent_id` bigint(20) DEFAULT NULL COMMENT '上级id',
   `view_permissions` bigint(20) DEFAULT '0' COMMENT '分类查看权限 0 全部 1 登录用户 2 支付积分(会员免费积分查看可以视为直接支付过积分) 4 会员',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的板块';
...
CREATE TABLE `bbs_appeal` (
  ...
  `appeal_file` varchar(800)  COMMENT '申诉附件',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `create_user_id` bigint(20) DEFAULT NULL COMMENT '创建用户',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `update_user_id` bigint(20) DEFAULT NULL COMMENT '更新用户',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的申诉';
...
CREATE TABLE `bbs_notice` (
  ...
  `appeal_text` varchar(500) NOT NULL COMMENT '公告文字',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `create_user_id` bigint(20) DEFAULT NULL COMMENT '创建用户',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `update_user_id` bigint(20) DEFAULT NULL COMMENT '更新用户',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的公告';
**/

DROP TABLE
IF
EXISTS `bbs_score_role`;
CREATE TABLE `bbs_score_role` (
  ...
  `score` int(11) DEFAULT NULL COMMENT '积分数量 0 不限 ',
  ...
  `version` bigint(20) DEFAULT '0' COMMENT '乐观锁',
  PRIMARY KEY (`role_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的积分规则配置';


DROP TABLE
IF
EXISTS `bbs_score_log`;
CREATE TABLE `bbs_score_log` (
  `score_log_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '积分日志主键',
   `role_id` bigint(20) NOT NULL COMMENT '积分规则',
   `score_count` int(11) DEFAULT NULL COMMENT '具体加的分数 ',
  `event_id` bigint(20) NOT NULL COMMENT '事件id',
  `return_msg` varchar(200) NOT NULL COMMENT '返回内容',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `create_user_id` bigint(20) DEFAULT NULL COMMENT '创建用户',
  PRIMARY KEY (`score_log_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的积分日志';

DROP TABLE
IF
        EXISTS `bbs_score_record`;
...
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '论坛的积分记录';



DROP TABLE
IF
EXISTS `bbs_punishment`;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的惩罚';
...
CREATE TABLE `bbs_message` (
        ...
        `tpis_type` TINYINT ( 4 ) NOT NULL COMMENT ' 通知类型，1动态，2评论，3回复，4私信，99系统通知',
        `message_event_id` bigint ( 20 ) NOT NULL COMMENT '产生消息的对应内容的主键',
        `sender_user_id` bigint ( 20 ) NOT NULL COMMENT '接收方用户ID',
...
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '论坛的消息';



DROP TABLE
IF
        EXISTS `bbs_invitation`;
CREATE TABLE `bbs_invitation` (
        ...
        `collection_cont` BIGINT ( 20 ) COMMENT '收藏数',
...
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '论坛的帖子';


/** 暂定 点赞---> 取消---> 再点赞 为两条 取消删除旧的 再点插入新的**/
DROP TABLE
IF
        EXISTS `bbs_invitation_thumbs_log`;
...
DROP TABLE
IF
        EXISTS `bbs_invitation_collection_log`;
CREATE TABLE `bbs_invitation_collection_log` (
        `collection_log_id` BIGINT ( 20 ) NOT NULL AUTO_INCREMENT COMMENT '收藏主键',
        `event_id` BIGINT ( 20 ) NOT NULL COMMENT '收藏内容的逐渐',
        `collection_type` TINYINT ( 4 ) DEFAULT '0' COMMENT '收藏类型 1 帖子 2 板块 3 用户动态',
        `create_time` datetime DEFAULT NULL COMMENT '创建时间',
        `create_user_id` BIGINT ( 20 ) DEFAULT NULL COMMENT '创建用户',
        `deleted` TINYINT ( 4 ) DEFAULT '0' COMMENT '逻辑删除标记位',
        `version` BIGINT ( 20 ) DEFAULT '0' COMMENT '乐观锁',
...
CREATE TABLE `bbs_comment` (
        ...
        `event_id` BIGINT ( 20 ) NOT NULL COMMENT '回复的内容对应级别的id',
        `event_level_path` BIGINT ( 20 ) NOT NULL COMMENT '回复帖子为空如果是回复评论,整个评论id的层级路径',
...
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '论坛的帖子评论';

DROP TABLE
IF
        EXISTS `bbs_comment_thumbs_log`;
...
CREATE TABLE `bbs_feedback_record` (
        ...
        `create_time` datetime DEFAULT NULL COMMENT '创建时间',
        `create_user_id` BIGINT ( 20 ) DEFAULT NULL COMMENT '创建用户',
PRIMARY KEY ( `feedback_record_id` ) 

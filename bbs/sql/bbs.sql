...
CREATE TABLE `bbs_invitation` (
  ...
  `ip_loc` varchar(64) NOT NULL DEFAULT '' COMMENT 'IP城市地址',
  `thumbs_cont` bigint(20) DEFAULT NULL COMMENT '点赞数',
  `collection_cont` bigint(20) DEFAULT NULL COMMENT '收藏数',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='论坛的收藏日志';
...
CREATE TABLE `bbs_invitation_thumbs_log` (
  ...
  `version` bigint(20) DEFAULT '0' COMMENT '乐观锁',
  PRIMARY KEY (`thumbs_log_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='论坛的点赞日志';
...
CREATE TABLE `bbs_message` (
  ...
  `tips_type` tinyint(4) NOT NULL COMMENT ' 通知类型，1动态，2评论，3回复，4私信，99系统通知',
...
CREATE TABLE `bbs_score_log` (
  ...
  `score_count` bigint(20) DEFAULT NULL COMMENT '具体加的分数 ',
  `event_id` bigint(20) NOT NULL COMMENT '事件id',
  `return_msg` varchar(200) NOT NULL COMMENT '返回内容',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `create_user_id` bigint(20) DEFAULT NULL COMMENT '创建用户',
  PRIMARY KEY (`score_log_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='论坛的积分日志';
...
CREATE TABLE `bbs_tag` (
  ...
  `deleted_time` bigint(20) unsigned NOT NULL DEFAULT '0' COMMENT '删除时间',
...
-- ----------------------------
-- Table structure for bbs_tips
-- ----------------------------
DROP TABLE IF EXISTS `bbs_tips`;
CREATE TABLE `bbs_tips` (
  `tips_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '站内信主键',
  `tpis_type` tinyint(4) NOT NULL COMMENT '提示类型 1 帖子回复通知 2 评论回复通知',
  `from_id` bigint(20) NOT NULL COMMENT '回复的帖子或者评论的id',
  `reply_id` bigint(20) NOT NULL COMMENT '回复的内容的id',
  `tips_status` tinyint(4) NOT NULL COMMENT '站内信状态 0 未读 1 已读',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `deleted` tinyint(4) DEFAULT '0' COMMENT '逻辑删除标记位',
  PRIMARY KEY (`tips_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='论坛的站内信';
